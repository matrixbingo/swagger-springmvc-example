...
public class EmployeeApi {
    ...
    @ApiOperation(value = "2121212", notes = "4434242424",response = Employee.class)
    @RequestMapping(value = "/employeesqwqw/{id}", method = RequestMethod.GET)
    public ResponseEntity<Employee> getEmployeeByIqqqqqd(@ApiParam(name = "id", value = "43432424", required = true) @PathVariable("id") int id) {
        if (id <= 3) {
            Employee employee = new Employee(1,"Lokesh","Gupta","howtodoinjava@gmail.com");
            return new ResponseEntity<Employee>(employee, HttpStatus.OK);
        }
